...
export const getTokensWithMostPools = async (chainId: number, version: Version): Promise<
    PoolTokensWithMostPools[]
> =>
    controlledAxiosPromise(
        backendInstance.get(ROUTES.TOKENS_WITH_MOST_POOLS_ROUTE + '?' + query(chainId, version))
    );
